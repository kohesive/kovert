...
class PublicAccess(private val routingContext: RoutingContext) {
    ...
    public val user: User? = routingContext.user() as? uy.kohesive.kovert.vertx.sample.services.User

    // allow access to functions for logged in user on public pages, if logged in, otherwise this will blow up with exception
    public fun upgradeToSecured(): UserSessionSecured = UserSessionSecured(routingContext)
}
...
class UserSessionSecured(private val routingContext: RoutingContext) {
    ...
    public val user: User = routingContext.user() as? uy.kohesive.kovert.vertx.sample.services.User ?: throw HttpErrorUnauthorized()
